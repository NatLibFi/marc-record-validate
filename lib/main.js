    ...
    return function(configuration)
    {
      ...
      return function(record)
      {
        ...
        validators_run.some(function(validator) {
          ...
          var results_validator = {
            name: validator.name,
            validate: validator.validate(record)
          };
                   
          if (validator.hasOwnProperty('description')) {
            results_validator.description = validator.description;
          }
 
          validateValidateResults(results_validator.validate);
          
          if (results_validator.validate.some(function(result) {
          ...
          } else if (configuration.fix && typeof validator.fix === 'function' && results_validator.validate.some(function(result) {
            return result.type === 'warning';
          })) {
            
            results_validator.fix = validator.fix(record);
            
            validateFixResults(results_validator.fix);
            
          }
          
          results.validators.push(results_validator);
