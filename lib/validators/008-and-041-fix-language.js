  ...
  return {
    validate: function(record)
    {
      return getFields(record).map(function(field) {
        return {
          ...
          messages: [{
            type: 'warning',
            message: 'Language code should be modified'
          }]
        };
      });
    },
    fix: function(record)
    {
      ...
      function fix008(field)
      {
        ...
        field.value = field.value.substring(0, 35) + lang_code_value + field.value.substring(38);
      ...
      function fix041(field)
      {
        ...
        field.subfields.forEach(function(subfield) {
          if (LANGUAGE_SUBFIELD_CODES_041.indexOf(subfield.code) >= 0 && LANGUAGE_CODES_MAP.hasOwnProperty(subfield.value)) {
            ...
            modifications.push({
              type: 'modifySubfield',
              oldValue: subfield.value,
              newValue: LANGUAGE_CODES_MAP[subfield.value]
            });
            
            field.subfield.value = LANGUAGE_CODES_MAP[subfield.value]; 
