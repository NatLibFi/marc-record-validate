  ...
  return {
    validate: function(record)
    {
      return getFields(record).map(function(field) {        
        return {
          ...
          messages: [{
            type: 'warning',
            message: 'Language code should be modified in 008/15-17'
          }]
        };
      });
    },
    fix: function(record) {
      return getFields(record).map(function(field) {
        ...
        country_code_new = COUNTRY_CODES_MAP[country_code];
        
        field.value = field.value.substring(0, 15) + country_code_new + field.value.substring(18);

        return {
          tag: field.tag,
          modifications: [{
            type: 'modifyField',
            oldValue: country_code,
            newValue: country_code_new
          }]
        };
