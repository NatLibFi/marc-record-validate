  ...
  return {
    validate: function(record) {
      return getFields(record).map(function(field) {
        return {
          ...
          messages: [{
            type: 'warning',
            message: "Fix $a '" + MISSING_MEDIA_TYPE_STRING + "'"
          }]
        };
      });
    },
    fix: function(record) {
      return getFields(record).map(function(field) {
        ...
        var subfield_a = shim_array.find(field.subfields, function(subfield) {
          return subfield.code === 'a';
        }),
        value_original = subfield_a.value;

        subfield_a.value = MISSING_MEDIA_TYPE_STRING;
        
        return {
          tag: field.tag,
          modifications: [{
            type: 'modifySubfield',
            oldValue: value_origin,
            newValue: subfield_a.value
          }]
        };
