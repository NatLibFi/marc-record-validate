...
function factory(Object)
{
  ...
  return {
    ...
    fix: {
      ...
      moveField: function(record, field, index_new)
      {

        var index_current = record.fields.indexOf(field);

        if (index_new >= record.fields.length) {
          record.fields.push(field);
        } else {
          record.fields.splice(index_new, 0, field);
        }

        if (index_current < index_new) {
          record.fields.splice(record.fields.indexOf(field), 1);
        } else {
          record.fields.splice(record.fields.lastIndexOf(field), 1);
        }
        ...
        return {
          ...
          'field': clone(field),
          'old': index_current,
          'new': record.fields.indexOf(field)
        };
      ...
      modifyFieldIndicator: function(field, indicator_number, indicator_value)
      {

        var field_original;

        if (indicator_number < 1 || indicator_number > 2) {
          throw new Error('Invalid indicator number: ' + indicator_number);
        } else {

          field_original = clone(field);          
          field['ind' + indicator_number] = indicator_value;
